...
- `components/TheCube.vue` is the central widget: a controlled component (`v-model:side`) that handles click-to-spin by picking a random next side (`getRandomAvailableSideIndex`) and emitting `update:side`, and renders six named slots (`front`/`back`/`left`/`right`/`top`/`bottom`) — each wrapped with a `SparklesSparkles` of a distinct `type`. 3D transforms on `.cube[data-side="…"]` drive the rotation.
