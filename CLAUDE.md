...
- `components/ui/` (`Button`, `Card`, `Badge`, `Layer`) are shared primitives consumed via Nuxt auto-import as `UiButton`, `UiCard`, `UiBadge`, `UiLayer` (the `Ui` prefix comes from the `ui/` subfolder, per Nuxt's component-name convention). Each lifts itself one elevation level (`translateZ(var(--elevation-step))`) above its parent surface inside the cube's 3D context; `UiLayer` is a plain wrapper that exists only for that lift (e.g. card text above the card's frosted overlay).
